...
# ---------------- MAIN APP ----------------
    ...
    if analyze:
        if user_input.strip() == "":
            ...
            cols = st.columns(3)
            max_role_score = top_roles['match_score'].max()  # normalize to 1-10
            for i, col in enumerate(cols):
                if i < len(top_roles):
                    normalized_score = round(1 + (top_roles.iloc[i]['match_score'] / max_role_score) * 9, 2)
                    col.markdown(f"""
                    <div style="background-color:#1e1e1e; padding:15px; border-radius:12px; text-align:center;">
                        ...
                        <p style="color:#00ffcc; font-size:16px;">Score: {normalized_score}</p>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            # ----------------------------
            # Scale scores to 1-10
            max_score = max([s for _, s in recommended])
            recommended_scaled = [(skill, round(1 + (score / max_score) * 9, 2)) for skill, score in recommended]
            skill_cards = pd.DataFrame(recommended_scaled, columns=['Skill','Score'])
            ...
            for i, col in enumerate(cols):
                if i < len(skill_cards):
                    ...
                    <div style="background-color:#1e1e1e; padding:10px; border-radius:12px; text-align:center;">
                        ...
                        <p style="color:#ffcc00;">Score: {skill_cards.loc[i,'Score']}</p>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            history = {
                "username": username,
                "skills": user_input,
                "recommended_skills": ", ".join([s.upper() for s,_ in recommended_scaled]),
